...
+ (NSString *)getAddressStringWithPublicStr:(NSString *)publicStr {
    NSData *publicData = [DBChainGMUtils hexToData:publicStr];
    // 生成地址
    Address *vc = [[Address alloc] init];
    NSString *addressStr = [vc sm2GetPubToDpAddress:publicData :ChainTypeCOSMOS_MAIN];
    NSLog(@"生成地址: %@",addressStr);
    return  addressStr;
}

+ (NSString *)getAddressStringWithPublicStr:(NSString *)publicStr chainType:(ChainType)type {
    NSData *publicData = [DBChainGMUtils hexToData:publicStr];
    // 生成地址
    Address *vc = [[Address alloc] init];
    NSString *addressStr = [vc sm2GetPubToDpAddress:publicData :type];
    NSLog(@"生成地址: %@",addressStr);
    return  addressStr;
}
